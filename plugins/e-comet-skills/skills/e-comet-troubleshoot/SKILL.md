...
3. The agent calls `e_comet_diagnose` with the narrowest scope for installation evidence, runtime extension context, or an exact operation receipt. Installation probes are `storage_write` and, only for an observed Codex host, `hook_permissions`; the runtime probe is `extension_snapshot`. All require `safe_probes`. Pairing has no probe: runtime diagnosis can only return the existing `pairingSource` observation. Repeating a passive read does not guarantee a more specific cause.
...
A Codex `hook_permissions` result is a configuration snapshot for the checked installation context, not proof that a hook ran in this task. `ready` means all expected native handlers are enabled and trusted. `disabled` means at least one trusted handler is off. `review_required` means at least one handler is new or changed and awaits review. For either failure, name the affected safe handler roles and direct the user to Codex `/hooks`; do not expose paths, commands, or hashes. Do not use a local Codex snapshot to explain Claude Code or Cowork.
